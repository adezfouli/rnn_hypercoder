...
def fix_init_all_vars(sess):
    ...
    for i in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
        agn.append(tf.assign(i, tf.random_uniform(tf.shape(i), minval=0.01, maxval=0.1, seed=1, dtype=i.dtype.base_dtype)))
    sess.run(agn)
...
def tf_cov(x):
    ...
    vx = tf.matmul(tf.transpose(x), x)/tf.cast(tf.shape(x)[0], Const.FLOAT)
    cov_xx = vx - mx
    return cov_xx
